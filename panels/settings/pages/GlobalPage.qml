...
NexusControls.PageBase {
    ...
    NexusControls.SliderRow { last: true; icon: "󰔎"; tint: Theme.tertiary; label: "Rounding"; from: 0; to: 40; stepSize: 1; unit: "px"; value: Theme.cornerRadius; onMoved: v => { Theme.setCornerRadius(Math.round(v)); UmbrielService.preview("cornerRadius", Math.round(v)) }; onApplied: v => { Theme.setCornerRadius(Math.round(v)); UmbrielService.applyCornerRadius(v) } }
