...
Singleton {
    ...
    FileView {
        id: volumeFile
        path: Quickshell.env("HOME") + "/.config/quickshell/solstice/backend/config/volume.json"
    ...
    Process {
        id: volProbe
        command: ["bash", "-c", Quickshell.env("HOME") + "/.config/quickshell/solstice/backend/scripts/volume.sh get 2>/dev/null | tr -d '\\n'"]
