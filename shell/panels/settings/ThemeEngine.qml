...
Item {
    ...
    FileView {
        id: themeEngineFile
        path: Quickshell.env("HOME") + "/.config/quickshell/solstice/style/themes/theme_engine.json"
    ...
    FileView {
        ...
        path: Quickshell.env("HOME") + "/.config/quickshell/solstice/style/themes/matugen_settings.json"
        watchChanges: true; onFileChanged: engineReloadDebounce.restart(); blockLoading: true; printErrors: false
    ...
    FileView {
        ...
        path: Quickshell.env("HOME") + "/.config/quickshell/solstice/style/themes/theme_wallpapers.json"
    ...
    function matugenBin(): string {
        ...
        return "RUN=\"$HOME/.config/quickshell/solstice/backend/scripts/matugen-run.sh\"; if [ -x \"$RUN\" ]; then MATUGEN=(bash \"$RUN\"); else [ -x \"$HOME/.cargo/bin/matugen\" ] && MATUGEN=(\"$HOME/.cargo/bin/matugen\") || MATUGEN=(matugen); fi;"
    }
    ...
    function runMonetApply(wallPath: string) {
        ...
        cmd += " [ -f \"$WALL\" ] || WALL=\"$(cat ~/.config/quickshell/solstice/backend/config/current_wallpaper.txt 2>/dev/null | tr -d '\\r\\n')\";"
        ...
        cmd += " (bash \"$HOME/.config/quickshell/solstice/backend/scripts/apply-gtk.sh\" \"$MODE\" 2>&1 | logger -t gtk) >/dev/null 2>&1 < /dev/null &"
    ...
    function runPresetApply(id: string, silent: bool) {
        ...
        let src = Quickshell.env("HOME") + "/.config/quickshell/solstice/style/themes/" + key + "-" + mode + ".json"
        let dst = Quickshell.env("HOME") + "/.config/quickshell/solstice/style/themes/matugen.json"
        let render = Quickshell.env("HOME") + "/.config/quickshell/solstice/backend/scripts/render-everforest.py"
        ...
        cmd += " (python3 \"$RENDER\" \"$MODE\" \"$SRC\" 2>&1 | logger -t \"$TAG\"; bash \"$HOME/.config/quickshell/solstice/backend/scripts/apply-gtk.sh\" \"$MODE\" 2>&1 | logger -t gtk) >/dev/null 2>&1 < /dev/null &"
